...
#+begin_src emacs-lisp
  ...
  (defun my/org-fade-done-subtrees ()
    "Fade DONE subtree contents and strip org-modern pills."
    (interactive)
    (save-excursion
      (save-restriction
        (widen)
        (remove-overlays (point-min) (point-max) 'my-org-done-fade t)
        (goto-char (point-min))
        (while (re-search-forward org-heading-regexp nil t)
          (when (string= (org-get-todo-state) "DONE")
            (let* ((element (org-element-at-point))
              ...
              (when (and begin end)
                ;; General fade for everything (this grays out the icons too)
                (let ((ov (make-overlay begin end)))
                  (overlay-put ov 'face `(:foreground ,my/org-done-fade-color))
                  (overlay-put ov 'priority 10)
                  (overlay-put ov 'my-org-done-fade t))

                ;; Specific fix for timestamps to kill org-modern black pills
                (save-excursion
                  (goto-char begin)
                  (while (re-search-forward org-ts-regexp-both end t)
                    (let* ((ts-start (match-beginning 0))
                           (ts-end (match-end 0))
                           (ts-text (buffer-substring-no-properties ts-start ts-end))
                           (ts-ov (make-overlay ts-start ts-end)))
                      (overlay-put ts-ov 'display ts-text)
                      (overlay-put ts-ov 'face `(:foreground ,my/org-done-fade-color :strike-through t))
  ...
  (add-hook 'org-mode-hook
            (lambda ()
              (add-hook 'after-save-hook #'my/org-fade-done-subtrees nil t)
              (add-hook 'org-after-todo-state-change-hook #'my/org-fade-done-subtrees nil t)
...
#+begin_src emacs-lisp
  ...
  (defun my/org-collapse-done-after-change ()
    "Collapse subtree immediately when task becomes DONE."
    (when (string= org-state "DONE")
      (save-excursion
        (org-back-to-heading t)
        (my/org-fold-subtree-keep-newlines))))

  ;; Integrated Hook Setup
  (add-hook 'org-mode-hook
            (lambda ()
              (add-hook 'org-after-todo-state-change-hook
                        #'my/org-collapse-done-after-change nil t)
...
#+begin_src emacs-lisp
  ...
  (defun my/org-compose-obsidian-prefix (limit)
    "Replace visible `obsidian:` with `my/obsidian-link-icon` via composition.
  Text & link properties remain unchanged, so links are still clickable."
    (when (re-search-forward "\\(\\bobsidian:\\)" limit t)
      (let ((start (match-beginning 1))
            (end   (match-end 1)))
        ;; visually replace only — underlying text remains `obsidian:`
