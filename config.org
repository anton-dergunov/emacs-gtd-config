...
** Display emojis for tasks in Org Agenda to grasp them easier visually
#+begin_src emacs-lisp
(require 'json)

(defvar my/emoji-matcher-path
  (expand-file-name "~/.emacs.d/emoji_matcher.py")
...
(defun my/extract-agenda-tasks ()
  "Extract all visible tasks from the *Org Agenda* buffer."
  (let (tasks)
    (save-excursion
      (goto-char (point-min))
      (while (re-search-forward
              "^[[:space:]]*\\(?:[^:]+:\\)?[[:space:]]+\\(TODO\\|NEXT\\|IN-PROGRESS\\)[[:space:]]+\\(.*\\)$"
              nil t)
        (let ((task (match-string 2)))
          ;; Clean up tags at the end of line like ":work:"
...
(defun my/run-emoji-matcher (tasks callback)
  ...
  (let ((buf (generate-new-buffer " *emoji-matcher-output*")))
    (make-process
     ...
     :buffer buf
     :command (list "/Library/Frameworks/Python.framework/Versions/3.10/bin/python3"
                    my/emoji-matcher-path)
     :noquery t
         ...
         (when (buffer-live-p (process-buffer p))
           (with-current-buffer (process-buffer p)
             (let* ((raw (buffer-string))
               ...
               (when parsed (funcall callback parsed))))
           (kill-buffer (process-buffer p))))))
    ;; Send JSON input
    (process-send-string "emoji-matcher-proc" (concat (json-encode tasks) "\n"))
    (process-send-eof "emoji-matcher-proc")))

(defun my/org-agenda-append-emojis ()
  "Append emojis to agenda tasks using external Python script."
  (interactive)
...
(defun my/org-agenda-append-emojis-debounced (&rest _)
  "Run emoji updater with small delay after agenda renders."
  ...
  (setq my/org-agenda-emoji-timer
        (run-with-idle-timer
         ...
         (lambda ()
           ...
           (my/org-agenda-append-emojis)))))

(advice-add 'org-agenda-finalize :after #'my/org-agenda-append-emojis-debounced)
#+end_src
