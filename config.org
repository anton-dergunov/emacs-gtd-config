...
#+begin_src emacs-lisp
  ...
  (defun my/org--clean-title (s)
    "Remove non-printable characters from string S."
  ...
  (defun my/org-insert-link-async ()
    "Insert an Org link from URL.
  Spoofs a browser User-Agent to bypass bot protections.
  Falls back to a raw URL if the title cannot be fetched."
    (interactive)
    (let* ((url-at-point-bounds (bounds-of-thing-at-point 'url))
           ...
           (maybe-url (or url-at-point
                          ...
                          (read-string "URL: ")))
           ;; SPOOF USER AGENT HERE: Look like Chrome instead of Emacs
                           ...
                           (let* ((html (buffer-substring-no-properties (point) (point-max)))
                             ...
                             (setq title
                                   (when title
                                     (string-trim
                                      (my/org--clean-title
                                       (condition-case _ (xml-substitute-special-chars title) (error title))))))
