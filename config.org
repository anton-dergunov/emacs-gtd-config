...
#+begin_src emacs-lisp
  ...
  (defun my/org-fade-done-subtrees ()
    "Fade DONE subtree contents, including stripping org-modern's timestamp pills."
    (interactive)
    (save-excursion
      (save-restriction
        (widen)
        ;; 1. Clear old fade overlays
        (remove-overlays (point-min) (point-max) 'my-org-done-fade t)

        (goto-char (point-min))
        ;; 2. Loop through headlines
...
#+begin_src emacs-lisp
  ...
  (defun my/org-compose-scheduled-deadline (limit)
    "Visually replace SCHEDULED and DEADLINE keywords with icons."
    (when (re-search-forward "\\<\\(SCHEDULED:\\|DEADLINE:\\)\\>" limit t)
      (let ((start (match-beginning 1))
            (end (match-end 1)))
        (compose-region
         start end
         (if (string= (match-string 1) "SCHEDULED:")
