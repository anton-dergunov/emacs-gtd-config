...
#+begin_src emacs-lisp
  (defvar my/org-scheduled-icon "⏱ "
    "Icon used to replace SCHEDULED keyword.")

  (defvar my/org-deadline-icon "⚑ "
    "Icon used to replace DEADLINE keyword.")

  (defun my/org-compose-scheduled-deadline (limit)
    "Visually replace SCHEDULED: and DEADLINE: with dark icons."
    ...
    (while (re-search-forward "\\(SCHEDULED:\\|DEADLINE:\\)" limit t)
      (let* ((start (match-beginning 1))
             ...
             (icon (if (string= label "SCHEDULED:")
                       my/org-scheduled-icon
                     my/org-deadline-icon)))
        ;; We apply the icon and a 'dark black' face.
        ;; The 'Fade' overlay will override this color for DONE tasks automatically.
        (add-text-properties start end
                             `(display ,icon
                                       face (:foreground "#000000" :weight bold)))))
    nil)

  (font-lock-add-keywords
   'org-mode
   '((my/org-compose-scheduled-deadline 0 t)))
#+end_src

** Fade DONE tasks
#+begin_src emacs-lisp
  (defvar my/org-done-fade-color "gray75"
    "Color used to fade DONE tasks.")

  (defun my/org-clear-done-fade-overlays ()
    "Remove all DONE fade overlays."
    (remove-overlays (point-min)
                     (point-max)
                     'my-org-done-fade t))

  (defun my/org-fade-done-subtrees (&rest _)
    "Fade DONE subtree contents and strip org-modern pills."
    (interactive)

    (when (derived-mode-p 'org-mode)

      ;; Prevent overlay accumulation
      (my/org-clear-done-fade-overlays)
              ...
              (let* ((begin
                ...
                (when (< begin end)
                  ...
                  (let ((ov (make-overlay begin end)))
                    (overlay-put ov
                                 'face
                                 `(:foreground ,my/org-done-fade-color))
                    (overlay-put ov 'priority 10)
                    (overlay-put ov 'my-org-done-fade t))
                  ...
                  (save-excursion
                    ...
                    (while (re-search-forward
                      ...
                      (let* ((ts-start (match-beginning 0))
                        ...
                        (overlay-put ts-ov 'my-org-done-fade t))))))))))))
  ...
  (defun my/org-refresh-after-revert ()
    ...
    (when (derived-mode-p 'org-mode)
      ...
      ;; Rebuild folds
      (my/org-collapse-done-subtrees)

      ;; Rebuild fades
      (my/org-fade-done-subtrees)

      ;; Final redraw
      (redisplay)))

  (defun my/org-after-todo-change-refresh ()
    "Refresh visuals after TODO state changes."
    (my/org-fade-done-subtrees)
    (when (string= org-state "DONE")
      (save-excursion
        (org-back-to-heading t)
        (my/org-fold-subtree-keep-newlines))))
  ...
  (add-hook
   ...
   (lambda ()
     ...
     (add-hook
      ...
      #'my/org-after-todo-change-refresh
      nil t)

     ;; Rebuild after auto-revert
     (add-hook
      'after-revert-hook
      #'my/org-refresh-after-revert
      nil t)
...
** Automatically collapse DONE tasks
#+begin_src emacs-lisp
  ...
  (defun my/org-fold-subtree-keep-newlines ()
  ...
  (defun my/org-collapse-done-subtrees ()
...
(defvar my/emoji-matcher-path
  (expand-file-name "scripts/org_emoji_matcher.py"
...
#+begin_src emacs-lisp
  ...
  (defcustom my/obsidian-link-icon "✎ "
    "Visual replacement for the `obsidian:` prefix in Org-mode.
  This affects only display, not the actual link text.")
  ...
  ;; Register rewrite rule in Org font-lock
  (font-lock-add-keywords
   'org-mode
  ...
  ;; Insert Obsidian link from clipboard
  (defun my/insert-obsidian-link-from-clipboard ()
  ...
  (global-set-key (kbd "<f4>") #'my/insert-obsidian-link-from-clipboard)

  ;; Insert Obsidian link by prompting
  (defun my/insert-obsidian-link-prompt ()
  ...
  (global-set-key (kbd "<f5>") #'my/insert-obsidian-link-prompt)
#+end_src
...
#+begin_src emacs-lisp
  ...
  (defun my/org--shorten (s)
    (if (and s (> (length s) my/org-link-title-max-length))
        (concat (substring s 0 my/org-link-title-max-length) "...")
      s))
        ...
        (while (and (string-match "&\\([^;]+\\);" decoded) (< limit 10))
          (let* ((entity (match-string 1 decoded))
            ...
            (setq decoded (if replacement
                              (replace-match replacement t t decoded)
  ...
  (defun my/org--system-clipboard-url ()
  ...
  (defun my/org--replace-marked-region-with (start-marker end-marker text)
  ...
  (defun my/org-insert-link-async ()
                           ...
                           (progn
                             (when (buffer-live-p orig-buf)
                               (with-current-buffer orig-buf
                                 ;; Fallback to RAW URL
                                 (my/org--replace-marked-region-with start-marker end-marker maybe-url)))
                                  ...
                                  (title
                                   (or
                                    ...
                                    (when (string-match "<title[^>]*>\\([^<]+\\)</title>" html)
                                      (match-string 1 html)))))
                             (setq title (my/org--clean-title title))
                             (if (and title (not (string-empty-p title)))
                                 (let ((final (my/org--shorten title)))
                                   (when (buffer-live-p orig-buf)
                                     (with-current-buffer orig-buf
                                       (my/org--replace-marked-region-with start-marker end-marker (format "[[%s][%s]]" maybe-url final)))))
                               ;; Fallback to RAW URL
                               (when (buffer-live-p orig-buf)
                                 (with-current-buffer orig-buf
                                   (my/org--replace-marked-region-with start-marker end-marker maybe-url)))
  ...
  ;; Now that the command is defined, bind it to F6
  (global-set-key (kbd "<f6>") #'my/org-insert-link-async)
#+end_src
...
#+begin_src emacs-lisp
  ...
  (define-key my-menu-bar-menu [my-cmd4]
              '(menu-item "Insert URL (auto-title)" my/org-insert-link-async :help "Insert URL (auto-title)"))
  (define-key my-menu-bar-menu [my-cmd3]
              '(menu-item "Insert Obsidian Link (prompt)" my/insert-obsidian-link-prompt :help "Insert [[obsidian:Title]] by typing title"))
  (define-key my-menu-bar-menu [my-cmd2]
              '(menu-item "Insert Obsidian Link (clipboard)" my/insert-obsidian-link-from-clipboard :help "Insert [[obsidian:Title]] using clipboard"))
  (define-key my-menu-bar-menu [my-cmd1]
              '(menu-item "Collapse DONE tasks" my/org-collapse-done))
  (define-key my-menu-bar-menu [my-cmd0]
              '(menu-item "Expand DONE tasks" my/org-expand-done))
...
#+begin_src emacs-lisp
  ...
  (defvar my/org-git-icon-ok "⇅")
  (defvar my/org-git-icon-syncing "↻")
  (defvar my/org-git-icon-offline "◌")
  (defvar my/org-git-icon-error "×")

  ;; ------------------------------------------------------------
  ;; Configuration
  ;; ------------------------------------------------------------

  (defvar my/org-git-sync-interval 60)

  (defvar my/org-git-sync-timer nil)
  (defvar my/org-git-sync-running nil)
  (defvar my/org-git-sync-paused nil)

  (defvar my/org-git-sync-last-status
    my/org-git-icon-offline)

  (defvar my/org-git-sync-last-message "")

  ;; ------------------------------------------------------------
  ;; Modeline
  ;; ------------------------------------------------------------

  (defun my/org-git-sync-modeline ()
    (propertize
     (format " %s" my/org-git-sync-last-status)
     'help-echo my/org-git-sync-last-message
     'face
     (cond
      (my/org-git-sync-paused
       '(:foreground "firebrick"))
      ((equal my/org-git-sync-last-status
              my/org-git-icon-ok)
       '(:foreground "gray40"))
      (t
       '(:foreground "gray60")))))

  (unless (member '(:eval (my/org-git-sync-modeline))
                  global-mode-string)
    (setq global-mode-string
          (append global-mode-string
                  '((:eval (my/org-git-sync-modeline))))))
  ...
  (defun my/org-git-root ()
    (when (and (boundp 'my-org-base-directory)
               (file-directory-p my-org-base-directory))
      (let ((default-directory my-org-base-directory))
        (string-trim
         (shell-command-to-string
          "git rev-parse --show-toplevel 2>/dev/null")))))

  (defun my/org-git-inside-repo-p ()
    (let ((root (my/org-git-root)))
      (and root
           (not (string-empty-p root)))))

  (defun my/org-git-set-status (icon message)
    (setq my/org-git-sync-last-status icon)
    (setq my/org-git-sync-last-message message)
  ...
  (defun my/org-git-handle-conflict (output)
    (setq my/org-git-sync-paused t)

    (my/org-git-set-status
     my/org-git-icon-error
  ...
  (defun my/org-git-sync ()
    (when (and
           (not my/org-git-sync-running)
           (not my/org-git-sync-paused)
           (my/org-git-inside-repo-p))

      (setq my/org-git-sync-running t)

      (my/org-git-set-status
       my/org-git-icon-syncing
        ...
        (make-process
         ...
         (lambda (proc _event)
           ...
           (when (memq (process-status proc)
             ...
             (setq my/org-git-sync-running nil)
             ...
             (let ((output
               ...
               (if (= (process-exit-status proc) 0)
                 ...
                 (if (string-match-p
                     ...
                     ;; serious issue
                     (my/org-git-handle-conflict output)
  ...
  (when (my/org-git-inside-repo-p)

    (when my/org-git-sync-timer
      (cancel-timer my/org-git-sync-timer))

    (setq my/org-git-sync-timer
          (run-with-timer
           10
           my/org-git-sync-interval
           #'my/org-git-sync)))
  ...
  (defun my/org-git-sync-resume ()
    (interactive)

    (setq my/org-git-sync-paused nil)
